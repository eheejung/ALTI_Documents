...
| Unique 제약                  |                      O                       |                       O                       |                                                              |
| Foreign Key 제약             |                      O                       |                       O                       |                                                              |
| Index                        |                      O                       |                       O                       | CUBRID의 Reverse index와 Prefix length index는 <br />Altibase에서 지원하지 않는다. <br />Reverse index는 인덱스 생성시 키 값을 역으로 <br />넣는 방식으로,  <br />Altibase 마이그레이션에서 지원하지 않는다. <br />Prefix length index는 키 값의 일정 부분만을 <br />인덱싱하는 기법으로, <br />마이그레이션시 Altibase의 일반 인덱스로 대체된다. |
