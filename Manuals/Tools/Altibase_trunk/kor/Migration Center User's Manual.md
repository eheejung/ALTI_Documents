...
###### 설명
%TYPE을 참조하는 변수의 데이터 타입이 VARRAY 또는 사용자 정의
  타입이라면, 수동으로 변환해야 한다.
...
###### 설명
PRAGMA가 제거되었다. 단 제거할 때 Altibase 6.3.1.0.10 이상 버전에서는 AUTONOMOUS_TRANSACTION을, Altibase 6.5.1.0.0 이상 버전에서는 EXCEPTION_INIT을 제외한다.
...
###### 원본 SQL 문장Altibase 6.3.1.0.10 이상 버전
~~~sql
~~~
  CREATE PROCEDURE addShot(p_cnt INTEGER)
...
###### 원본 SQL 문장Altibase 6.5.1.0.0 이상 버전
~~~sql
~~~
...
> 이 규칙은 Altibase 6.5.1.0.0 이전 버전에 적용된다.
...
> 이 규칙은 Altibase 6.3.1.0.0 이상 버전에 적용된다.
...
​		Native library를 사용하는 TimesTen type 2 JDBC driver를 로딩한 상태에서 so 파일을 다시 로딩할 때 Java classloader에서 발생하는 예외상황이다. 예를 들		어, 연결 테스트를 수행한 후 다른 JDBC Driver를 선택하여 다시 테스트할 때 발생한다. Java Native Interface (JNI) specification에 따르면 native library는 한		번만 로딩할 수 있다. 참고: https://docs.oracle.com/javase/1.5.0/docs/guide/jni/spec/invocation.html#library_version
