...
| Index                       |                      O                       |                       O                       |                                                              |
| Sequence                    |                      O                       |                       X                       |                                                              |
| Queue                       |                      X                       |                       X                       | 변환 가능한 객체가 없기 때문에, build 단계에서 <br />자동으로<br />제외된다. |
| Private Synonym             |                  부분 지원                   |                       X                       | 다른 schema 내의 객체를 참조하는 시노님도 <br />마이그레이션된다. |
...
| Unique 제약              |                   O                    |                    O                    |                                                              |
| Foreign Key 제약         |                   O                    |                    O                    |                                                              |
| Index                    |                   O                    |                    O                    | CUBRID의 Reverse index와 Prefix length index는 Altibase에서 <br />지원하지 않는다. <br />Reverse index는 인덱스 생성시 키 값을 역으로 넣는 방식으로, <br />Altibase 마이그레이션에서 지원하지 않는다. Prefix length index는 <br />키 값의 일정 부분만을 인덱싱하는 기법으로, 마이그레이션시 Altibase의 일반 인덱스로 대체된다. |
