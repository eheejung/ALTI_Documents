...
Altibase SQL에서 아래의 문자 함수나 연산자를 사용할 때 정규 표현식을 사용할 수 있다.
...
| 문법   | 설명                                                         |
| :----- | :----------------------------------------------------------- |
| ?      | 앞의 문자가 0번 또는 1번 나오는 문자를 찾는다. 욕심많은(greedy) 수량자 |
| ?+     | 앞의 문자가 0번 또는 1번 나오는 문자를 찾는다. 독점전인(possessive) 수량자 |
| ??     | 앞의 문자가 0번 또는 1번 나오는 문자를 찾는다. 게으른(lazy) 수량자 |
| *      | 앞의 문자가 0 번 이상 등장하는 문자를 찾는다. 욕심많은(greedy) 수량자 |
| *+     | 앞의 문자가 0 번 이상 등장하는 문자를 찾는다. 독점전인(possessive) 수량자 |
| *?     | 앞의 문자가 0 번 이상 등장하는 문자를 찾는다. 게으른(lazy) 수량자 |
| +      | 앞의 문자가 1회 이상 나오는 문자를 찾는다. 욕심많은(greedy) 수량자 |
| ++     | 앞의 문자가 1회 이상 나오는 문자를 찾는다. 독점전인(possessive) 수량자 |
| +?     | 앞의 문자가 1회 이상 나오는 문자를 찾는다. 게으른(lazy) 수량자 |
| {n}    | 앞의 문자가 n회 나오는 문자를 찾는다.                        |
| {n,m}  | 앞의 문자가 최소 n번, 최대 m회 반복되는 문자를 찾는다. 욕심많은(greedy) 수량자 |
| {n,m}+ | 앞의 문자가 최소 n번, 최대 m회 반복되는 문자를 찾는다. 독점전인(possessive) 수량자 |
| {n,m}? | 앞의 문자가 최소 n번, 최대 m회 반복되는 문자를 찾는다. 게으른(lazy) 수량자 |
| {n,}   | 앞의 문자가 n회 이상 나오는 문자를 찾는다. 욕심많은(greedy) 수량자 |
| {n,}+  | 앞의 문자가 n회 이상 나오는 문자를 찾는다. 독점전인(possessive) 수량자 |
| {n,}?  | 앞의 문자가 n회 이상 나오는 문자를 찾는다. 게으른(lazy) 수량자 |
...
| -------------------- | :------------------------------------------------------------ | :------------------------------------------------------------ | :---------------------------------- |
| POSIX 문자 클래스 | `SELECT REGEXP_COUNT('ABCDEFG1234567abcdefgh!@#$%^&*(','[:punct:]+');` | `SELECT REGEXP_COUNT('ABCDEFG1234567abcdefgh!@#$%^&*(','[[:punct:]]+');` | POSIX 문자 클래스표현식이 다르다. |
