...
> Cause: An internal error occurred in PCRE2 library or while executing Altibase internal function.Action: Check the error message and contact Altibase's Support Center (http://support.altibase.com).
...
이 장에서 안내하는 에러 메시지는 0x2106c 에서 코드의 <1%s>에 발생하는 메시지이다. 
...
PCRE2 제약 사항으로 인하여 오류가 발생했습니다. 오류 메세지를 참고하여 정규식을 알맞게 수정해주세요. 구체적인 원인 확인이 필요한 경우 Altibase 고객지원 센터로 연락바랍니다.
...
입력된 문자열들의 인코딩이 올바른 UTF-8 인코딩을 가지고 있지 않습니다. 입력된 문자열들의 인코딩을 확인하여 UTF-8 인코딩에 알맞게 수정합니다.
