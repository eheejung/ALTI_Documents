...
aku 에서 생성하는 Altibase 이중화 객체은 *REPLICATION_NAME_PREFIX*_\[*파드번호*]\[*파드번호*\] 와 같은 규칙으로 생성된다. 쿠버네티스 스테이트풀셋은 *name*\_0, *name*\_1, ..., *name*\_N-1 순서로 순차적으로 파드를 생성하며 각 파드는 고유한 순번을 가진다. Altibase 이중화 객체 이름에서 파드 번호는 파드 이름에서 *name*\_ 뒤의 번호를 의미하며, 이중화 쌍이 되는 파드들의 번호들로 구성된다. 이해를 돕기 위해 AKU_SERVER_COUNT가 4이고 REPLICATION_NAME_PREFIX가 AKU_REP 일 때, 각 파드에서 생성되는 이중화 객체 이름을 살펴보자. 
...
✔️aku가 Altibase 이중화 객체는 사용자가 임의로 생성/삭제/수정하면 안 된다. 
...
| 파드 이름 | 이중화 객체 이름                                             |
| --------- | ------------------------------------------------------------ |
| 파드-0    | AKU_REP_01 : 파드-0과 파드-1의 Altibase 서버 간 이중화<br />AKU_REP_02 : 파드-0과 파드-2의 Altibase 서버 간 이중화<br />AKU_REP_03 : 파드-0과 파드-3의 Altibase 서버 간 이중화 |
| 파드-1    | AKU_REP_01 : 파드-0과 파드-1의 Altibase 서버 간 이중화<br />AKU_REP_12 : 파드-1과 파드-2의 Altibase 서버 간 이중화 <br />AKU_REP_13 : 파드-1과 파드-3의 Altibase 서버 간 이중화 |
| 파드-2    | AKU_REP_02 : 파드-0과 파드-2의 Altibase 서버 간 이중화<br />AKU_REP_12 : 파드-1과 파드-2의 Altibase 서버 간 이중화 <br />AKU_REP_23 : 파드-2과 파드-3의 Altibase 서버 간 이중화 |
| 파드-3    | AKU_REP_03 : 파드-0과 파드-3의 Altibase 서버 간 이중화<br />AKU_REP_13 : 파드-1과 파드-2의 Altibase 서버 간 이중화<br />AKU_REP_23 : 파드-2과 파드-3의 Altibase 서버 간 이중화 |
...
스케일링으로 파드를 추가하거나 삭제할 때 Altibase에서 수행할 작업을 명시한다. -p 또는 --pod 파라미터 뒤에 start, end, clean 중 하나를 반드시 입력해야 한다. aku 설정 파일에 입력한 이중화 대상 테이블은 aku -p 수행 전에 생성해야 한다. 
